 ...
 *  @author Kevin Wayne
 */
public class EdgeWeightedDigraph1 {
    private static final String NEWLINE = System.getProperty("line.separator");

    private final int V;                // number of vertices in this digraph
    private int E;                      // number of edges in this digraph
    private Bag<DirectedEdgelp>[] adj;    // adj[v] = adjacency list for vertex v
     ...
     */
    public EdgeWeightedDigraph1(int V) {
        if (V < 0) throw new IllegalArgumentException("Number of vertices in a Digraph must be nonnegative");
        this.V = V;
        this.E = 0;
        this.indegree = new int[V];
        adj = (Bag<DirectedEdgelp>[]) new Bag[V];
        for (int v = 0; v < V; v++)
            adj[v] = new Bag<DirectedEdgelp>();
    }
     ...
     */
    public EdgeWeightedDigraph1(In in) {
        this(in.readInt());
        int E = in.readInt();
        ...
        for (int i = 0; i < E; i++) {
            ...
            double custot = in.readDouble();
            float  custom = in.readFloat(); 
            addEdge(new DirectedEdgelp(v, w, distancia, custot, custom));
        }
    }
     ...
     */
    public EdgeWeightedDigraph1(EdgeWeightedDigraph1 G) {
        this(G.V());
        this.E = G.E();
        for (int v = 0; v < G.V(); v++)
        ...
        for (int v = 0; v < G.V(); v++) {
            ...
            Stack<DirectedEdgelp> reverse = new Stack<DirectedEdgelp>();
            for (DirectedEdgelp e : G.adj[v]) {
                reverse.push(e);
            }
            for (DirectedEdgelp e : reverse) {
                adj[v].add(e);
            }
        }
    }
     ...
     */
    public void addEdge(DirectedEdgelp e) {
        int v = e.from();
        int w = e.to();
        validateVertex(v);
        validateVertex(w);
     ...
     */
    public Iterable<DirectedEdgelp> adj(int v) {
        validateVertex(v);
        return adj[v];
    }
     ...
     */
    public Iterable<DirectedEdgelp> edges() {
        Bag<DirectedEdgelp> list = new Bag<DirectedEdgelp>();
        for (int v = 0; v < V; v++) {
            for (DirectedEdgelp e : adj(v)) {
                list.add(e);
            }
        }
        return list;
